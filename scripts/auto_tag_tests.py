...
def generate_skeleton_tests(df, output_dir):
    ...
    count = 0
    sev_map = {"q4":"CRITICAL","q3":"NORMAL","q2":"MINOR","q1":"TRIVIAL"}
    for _,row in df.iterrows():
        module = row["module"]; category = row.get("category","Unknown")
        quadrant = row.get("quadrant","Q2 - Test Third")
...
def tag_existing_tests(df, test_dir):
    mm = {r["module"].lower():r.get("quadrant","Q2").split(" ")[0].lower() for _,r in df.iterrows()}
    updated = 0
    for pf in Path(test_dir).glob("test_*.py"):
        content = pf.read_text()
        for mn,q in mm.items():
            if mn in content.lower():
                for old in re.findall(r"@pytest\.mark\.(q[1-4])", content):
...
def generate_summary(df, path="data/quadrant_summary.md"):
    ...
    lines = ["# PDR-I Risk-Based Testing: Quadrant Assignment\n",f"**Modules: {len(df)}**\n"]
    for ql,desc in [("Q4 - Test First","Every build"),("Q3 - Test Second","Daily"),
                    ("Q2 - Test Third","RC only"),("Q1 - Test Last","RC if time")]:
